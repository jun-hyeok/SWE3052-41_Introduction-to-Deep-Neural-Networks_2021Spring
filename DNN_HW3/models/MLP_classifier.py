...
class MLP_classifier(nn.Module):
    def __init__(self, input_dim, output_dim, learning_rate):
        ...
        self.fc3 = nn.Linear(
            in_features=n_size, out_features=self.output_dim, bias=True
        )
    ...
    def predict(self, x):
        ...
        with torch.no_grad():
            for i, batch_data in enumerate(data_loader):
                ...
                pred_y[start:end] = np.argmax(batch_pred_y.numpy(), -1).astype(int)
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            if epoch % print_every == 0:
                ...
                if best_acc < valid_acc:
                    print(
                        "Best Accuracy updated (%.4f => %.4f)" % (best_acc, valid_acc)
                    )
                    best_acc = valid_acc
                    best_epoch = epoch
